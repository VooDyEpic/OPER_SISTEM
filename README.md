...
Скрипт предназначен для автоматического поиска и резервного копирования графических файлов. Логика работы состоит из следующих этапов:

1. **Проверка аргументов**
   * Если не указан путь к целевой папке (первый аргумент), скрипт выводит сообщение об ошибке, демонстрирует пример корректного запуска и завершает работу.

2. **Проверка существования исходной папки**
   * Производится проверка переданного пути. Если указанный путь не существует или ведет не к директории (папке), скрипт выводит ошибку и завершает выполнение.

3. **Подготовка путей**
   * Скрипт определяет родительский каталог и имя исходной папки.
   * Автоматически формирует уникальное имя для будущей резервной копии в формате: `имя_папки_backup_дата-время`.

4. **Создание папки для резервной копии**
   * В родительском каталоге создается новая директория по подготовленному пути. Благодаря флагу `-Force`, если папка с точно таким же именем уже существовала, она будет перезаписана.

5. **Копирование файлов**
   * Скрипт выполняет рекурсивный поиск файлов во всех вложенных подпапках.
   * Ищутся файлы со следующими расширениями (регистронезависимо): `.jpg`, `.jpeg`, `.png`, `.gif`, `.heic`.
   * Каждый найденный файл копируется в созданную папку бэкапа, а в консоль выводится подробный лог (`Verbose`) о процессе копирования.
   * Параллельно ведется точный подсчет количества успешно скопированных файлов.

6. **Завершение работы**
   * **Если файлы не найдены:** скрипт удаляет созданную пустую папку резервной копии (чтобы не оставлять мусор на диске), выводит предупреждение и завершает работу.
   * **В случае успеха:** выводит в консоль итоговое количество скопированных изображений и точный полный путь к созданной резервной копии.
